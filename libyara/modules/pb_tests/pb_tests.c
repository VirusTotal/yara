    ...
    size_t module_data_size)
{
  ...
  for (int i = 0; i < pb->n_f_struct_array; i++) {
    ...
    if (pb->f_struct_array[i] != NULL) {
      set_string(pb->f_struct_array[i]->f_string, module_object, "f_struct_array[%i].f_string", i);

      if (pb->f_struct_array[i]->has_f_enum) {
        set_integer(pb->f_struct_array[i]->f_enum, module_object, "f_struct_array[%i].f_enum", i);
      }

      if (pb->f_struct_array[i]->f_nested_struct != NULL) {

        if (pb->f_struct_array[i]->f_nested_struct->has_f_int32) {
          set_integer(pb->f_struct_array[i]->f_nested_struct->f_int32, module_object, "f_struct_array[%i].f_nested_struct.f_int32", i);
        }
        set_string(pb->f_struct_array[i]->f_nested_struct->f_string, module_object, "f_struct_array[%i].f_nested_struct.f_string", i);
      }
      ...
      for (int j = 0; j < pb->f_struct_array[i]->n_f_nested_struct_array; j++) {
        ...
        if (pb->f_struct_array[i]->f_nested_struct_array[j] != NULL) {
          ...
          if (pb->f_struct_array[i]->f_nested_struct_array[j]->has_f_int32) {
            set_integer(pb->f_struct_array[i]->f_nested_struct_array[j]->f_int32, module_object, "f_struct_array[%i].f_nested_struct_array[%i].f_int32", i, j);
          }
          set_string(pb->f_struct_array[i]->f_nested_struct_array[j]->f_string, module_object, "f_struct_array[%i].f_nested_struct_array[%i].f_string", i, j);
        }
      }
    }
  }
