...
#define MODULE_NAME md5
#define MODULE_NAME_STR "md5"
#define MIN(X,Y) ((X) < (Y) ? (X) : (Y))


/// Start RSA code block from RFC 1321
...
///END RSA code block  code below is YARA code only
...
define_function(md5_hash)
{
  ...
  foreach_memory_block(context, block)
  {
    ...
    // if we need to offset within this block
    if (offset > file_pos) {
      read_length = MIN(offset, block->size);
      block_pos += read_length;  // update position within block
      file_pos += read_length;  // update position within
    ...
    read_length = MIN(block->size, length);
    length -= read_length;  // remove length from remaining length
    MD5_Update(&md5_context, block->data, read_length);

    file_pos +=read_length;
  }
...
declare_function("hash", "ii", "s", md5_hash)
