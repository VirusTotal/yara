...
the ``which_callback`` parameter to one of ``yara.CALLBACK_MATCHES``,
...
.. code-block:: python
  ...
  matches = rules.match('/foo/bar/my_file', callback=mycallback, which_callback=yara.CALLBACK_MATCHES)
